 ...
 * 【動作概要】
 * 1. init() で初期化
 *    - resetToDefaults() でヘッダファイルのデフォルト値をメモリにロード
 *    - SPIFFS に設定ファイルがあれば上書きロード
 *
 * 2. initDefaults(SettingManager&) でNVS初期化
 *    - 未設定のキーのみデフォルト値を投入
 *    - 既存設定は保持される（上書きしない）
 *
 * 3. Web UIから設定変更時
 *    - fromJson() でメモリ上の設定を更新
 *    - saveToSPIFFS() でファイルに永続化
 *
 * 【設定の優先順位】
 * SPIFFS設定ファイル > NVS > ヘッダのデフォルト値
 *
 * 【大量生産時のフロー】
 * 1. AraneaSettings.h のデフォルト値を施設用に編集
 * 2. ビルド
 * 3. 複数台に書き込み
 * 4. 各デバイスは初回起動時にデフォルト値をNVSに保存
 * 5. 以降はNVS/SPIFFSの値を使用
 */
