...
当初「波及影響」と表現したが、これは不正確。カメラ間の障害伝播は発生していない。
...
│                   PollingOrchestrator                        │
│  ┌─────────────────────┐   ┌─────────────────────┐          │
│  │  Subnet 125.x Loop  │   │  Subnet 126.x Loop  │ 並列    │
│  │   (独立実行)        │   │   (独立実行)        │          │
│  └──────────┬──────────┘   └──────────┬──────────┘          │
│             │                         │                       │
│             ▼                         ▼                       │
│     カメラ順次処理 (for camera in enabled)                    │
│     cam1 → cam2 → cam3 → ... (1台ずつawait)                 │
└──────────────────────────────────────────────────────────────┘
```
...
**■ 垂直依存（同一カメラ内）**:
```
ゾンビ蓄積 → 同一カメラの接続スロット消費 → 新規接続失敗
例: 125.19のゾンビ33個 → 125.19へのTIMEOUT 204回
```
- ゾンビプロセスは**自カメラのみ**に影響
- Tapoカメラの接続制限（2-4接続）を消費

**■ 水平独立（カメラ間）**:
```
125.19のゾンビ → 125.12には影響なし
125.12の障害原因: NO_ROUTE (ネットワーク到達不能)
```
- 各カメラの障害は**独立事象**
- 他カメラのゾンビが別カメラに影響することはない

**■ サブネット独立**:
```
125.xループの障害 → 126.xループに影響なし
126.xのエラー数: 極少（独立して正常動作）
```

### 10.4 エビデンス: エラータイプ別カメラ分布

| カメラIP | ゾンビ数 | 主要エラー | 解釈 |
|----------|----------|------------|------|
| 192.168.125.12 | 1 | NO_ROUTE (54) | ネットワーク到達不能 |
| 192.168.125.13 | 0 | NO_ROUTE (411) | ネットワーク到達不能 |
| 192.168.125.19 | 33 | TIMEOUT (204) | ゾンビによるスロット枯渇 |
| 192.168.125.45 | 0 | TIMEOUT (467) | カメラ応答遅延 |
| 192.168.125.62 | 1 | TIMEOUT (330) | カメラ応答遅延 |
| 192.168.125.79 | 0 | NO_ROUTE (285) | ネットワーク到達不能 |

**分析結果**:
- **NO_ROUTE系**: ネットワークレベルの問題（ゾンビと無関係）
- **TIMEOUT + 高ゾンビ (125.19)**: ゾンビによる自己影響
- **TIMEOUT + 低ゾンビ (125.45, 125.62)**: カメラ側の応答遅延

### 10.5 結論: 障害の独立性

1. **125.19の問題**: 自身のゾンビプロセスが接続スロットを消費
2. **125.12, 125.13, 125.79の問題**: ネットワーク到達不能（別原因）
3. **125.45, 125.62の問題**: カメラ応答遅延（別原因）
4. **126.xサブネット**: ほぼ正常動作（独立）

**「波及」は発生していない**。各カメラの障害は独立した原因を持つ。
ゾンビプロセスの影響は**同一カメラ内に限定**される。
...
- [ ] 125.12, 125.13, 125.79のネットワーク到達不能の原因調査
- [ ] 125.45, 125.62の応答遅延の原因調査（カメラ側負荷？）
