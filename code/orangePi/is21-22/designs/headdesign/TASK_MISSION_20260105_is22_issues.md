...
| Issue # | タイトル | 優先度 | 調査状況 |
|---------|---------|--------|----------|
| Issue 3 | サブネット別巡回時間表示 | P1（依存元） | ✅ 完了 (2026-01-06) |
...
#### 2.1.1 現象
- **期待動作**: ヘッダーに `125.0/24: MM:SS/cycle | 126.0/24: MM:SS/cycle` 形式でサブネット別巡回時間を表示
- **実際の動作**: 単一の `LIVE 巡回 01:10` 表示のみ
...
| ステップ | ファイル | 作業内容 |
|----------|----------|----------|
| 1 | `polling_orchestrator/mod.rs` | `subnet: subnet.clone()` を追加 |
| 2 | `useWebSocket.ts` | TypeScript interface に subnet を追加 |
| 3 | `App.tsx` | `Record<string, CycleState>` でサブネット別管理 |
| 4 | `App.tsx` | ヘッダー表示を `125.0/24: MM:SS | 126.0/24: MM:SS` 形式に変更 |
...
### 4.1 Issue 3 テスト項目 ✅ 完了

| # | テスト内容 | 検証方法 | 結果 |
|---|-----------|----------|------|
| T3-1 | バックエンドからsubnet付きCycleStatsMessageが送信される | WebSocketログ確認 | ✅ PASS |
| T3-2 | フロントエンドでサブネット別に状態管理される | React DevTools確認 | ✅ PASS |
| T3-3 | ヘッダーに `125.0/24: MM:SS | 126.0/24: MM:SS` 形式で表示 | ブラウザUI確認 | ✅ PASS |
| T3-4 | 各サブネットの時間が独立して更新される | 実機動作確認 | ✅ PASS |
