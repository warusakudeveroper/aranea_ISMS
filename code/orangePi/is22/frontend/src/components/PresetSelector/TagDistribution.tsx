            ...
            <div key={index} className="group relative">
              <div className="flex items-center justify-between text-xs mb-1">
                <div className="flex items-center gap-2 min-w-0">
                  <span
                    className={`truncate ${tag.is_over_detected ? 'text-red-600 dark:text-red-400 font-medium' : 'text-muted-foreground'}`}
                    title={tag.tag}
                  >
                    {tag.tag}
                  </span>
              ...
              {/* Tooltip on hover */}
              <div className="hidden group-hover:block absolute z-10 bg-popover border border-border rounded px-2 py-1 text-xs mt-1 shadow-lg">
                ...
                {tag.is_over_detected && (
                  <div className="text-red-600 dark:text-red-400">⚠️ 過剰検出の可能性</div>
                )}
              </div>
            </div>
          );
